...
const registrationBase = { fullName: z.string().trim().min(2).max(100), mobile: indianMobile, email: z.string().trim().email().toLowerCase().optional().or(z.literal('')), password, city: z.string().trim().min(2).max(80), state: z.string().trim().min(2).max(80), referredByCode: z.string().trim().toUpperCase().max(40).optional().or(z.literal('')), consent: z.literal(true, { errorMap: () => ({ message: 'Consent is required' }) }) };
