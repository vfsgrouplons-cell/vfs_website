...
export function ContactPage() {
  ...
  const settings = useSiteSettings(); const contact = settings.data?.contact; const phone = contact?.phone || '919160353295'; const whatsapp = contact?.whatsapp || phone; const emailReady = Boolean(contact?.email);
  ...
  const address = contact?.addressLines?.length ? [...contact.addressLines, contact.city, contact.state, contact.pinCode].filter(Boolean).join(', ') : 'Not published yet';
